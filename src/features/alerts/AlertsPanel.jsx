...
export default function AlertsPanel() {
  ...
  const devices = useSelector((state) => Object.values(state.devices.byId));

  const acknowledged = useSelector((state) => state.alerts.acknowledged);

  const alerts = devices.filter(
    (d) =>
      (d.state === DEVICE_STATES.FAULT || d.state === DEVICE_STATES.OFFLINE) &&
      !acknowledged[d.deviceId],
  );

  return (
    <div className="bg-slate-800 rounded-lg p-4 h-full">
      <h2 className="text-lg font-semibold text-red-400 mb-4">Active Alerts</h2>
          ...
          <tbody>
            {alerts.map((device, idx) => (
              ...
              >
                ...
                <td className="font-mono px-2">{device.deviceId}</td>
